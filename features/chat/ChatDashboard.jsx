...
const ChatDashboard = ({ selectedStoreIds, onLogout }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const selectedStoreNames = INITIAL_STORES.filter((s) =>
    ...
    <div className="flex h-screen overflow-hidden bg-background-light dark:bg-[#0f1411]">
      ...
      <aside className="w-72 bg-white dark:bg-zinc-900 flex flex-col justify-between p-6 border-r border-gray-100 dark:border-white/5 h-full shrink-0">
        <div className="flex flex-col gap-8 overflow-hidden">
          <div className="flex items-center gap-3 shrink-0">
            ...
            <h1 className="text-xl font-extrabold tracking-tight text-smart-dark dark:text-white">
              {APP_NAME}
            </h1>
          </div>
          ...
          <div className="flex flex-col gap-4 overflow-hidden">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-[11px] font-extrabold uppercase tracking-widest text-slate-gray/40 dark:text-gray-500">
                Selected Stores
              </h2>
            </div>
            <div className="flex flex-col gap-2.5 overflow-y-auto pr-1">
              {INITIAL_STORES.filter((s) => selectedStoreIds.has(s.id)).map(
                (store) => (
                  <div
                    key={store.id}
                    className="group flex items-center justify-between p-3 rounded-2xl bg-white dark:bg-zinc-800/50 shadow-sm border border-gray-100 dark:border-white/5 transition-all hover:border-primary/30"
                  >
                    <div className="flex items-center gap-3">
                      <div className="bg-gray-50 dark:bg-white/5 p-2 rounded-xl">
                        <span className="material-symbols-outlined text-sm text-slate-gray/60 dark:text-gray-400">
                          {store.icon}
                        </span>
                      </div>
                      <span className="text-sm font-bold text-smart-dark dark:text-white">
                        {store.name}
                      </span>
                    </div>
                  </div>
      ...
      <main className="flex-1 h-full flex flex-col items-center relative">
        <div className="max-w-[960px] w-full px-6 flex flex-col h-full">
          ...
          <div
            ref={scrollRef}
            className="flex-1 overflow-y-auto scroll-smooth py-8 flex flex-col gap-10 scrollbar-hide"
          >
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                {msg.sender === "ai" && (
                  <div className="flex items-center gap-2 px-1">
                    ...
                    <span className="text-[10px] font-extrabold text-slate-gray/40 dark:text-gray-500 uppercase tracking-widest">
                      SMARTBOT AI
                    </span>
                  </div>
                )}
                    ...
                    <div className="mt-6 flex flex-col gap-6 animate-slide-up">
                      <div className="flex flex-col md:flex-row gap-5">
                        <img
                          ...
                          src={
                            msg.comparisonData.productImage ||
                            `https://picsum.photos/seed/${msg.comparisonData.productName}/200`
                          }
                        />
                        <div className="flex-1 flex flex-col justify-center">
                          <p className="text-xs font-extrabold text-slate-gray/40 uppercase tracking-widest mb-1">
                            Comparing Prices For
                          </p>
                          <h4 className="text-xl font-extrabold text-smart-dark dark:text-white">
                            {msg.comparisonData.productName}
                          </h4>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {msg.comparisonData.stores.map((s, idx) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-white/5 group hover:border-primary/20 transition-all"
                          >
                            <div className="flex items-center gap-3">
                              <span className="material-symbols-outlined text-primary text-lg">
                                {s.storeIcon || "storefront"}
                              </span>
                              <span className="text-sm font-bold text-smart-dark dark:text-white">
                                {s.storeName}
                              </span>
                            ...
                            <span className="text-sm font-extrabold text-primary">
                              {s.currency || "RM"} {s.price.toFixed(2)}
                            </span>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="p-5 bg-[#f0fdf4] dark:bg-emerald-950/20 rounded-2xl border border-emerald-100 dark:border-emerald-900/30 flex items-start gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          <p className="text-[10px] font-extrabold text-emerald-800 dark:text-emerald-300 uppercase tracking-widest">
                            AI RECOMMENDATIONS:
                          </p>
                          <ul className="text-xs text-emerald-800 dark:text-emerald-300 leading-relaxed list-disc ml-4 space-y-1.5 font-medium">
                            {msg.comparisonData.recommendations.map((r, i) => (
                              <li key={i}>{r}</li>
                            ))}
                          </ul>
                        </div>
            ...
            {isLoading && (
              <div className="flex flex-col gap-3 items-start animate-pulse">
                <div className="flex items-center gap-2 px-1">
                  ...
                  <span className="text-[10px] font-extrabold text-slate-gray/40 uppercase tracking-widest">
